		...
		try {
			...
			mess = {
				...
				only: {
					...
					ownerG: '⚠️ Este comando só pode ser usado pelo dono do bot!(NINEM) 😂',
					ownerB: '❌ Este comando só pode ser usado pelo proprietário do bot! ❌',
					admin: '⚠️ Este comando só pode ser usado por admins! seu membro comum ksks 😝',
...
*nem eu entendi kzks*`)
				...
				case 'porno':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'não to lembrando de o adm ter liberado porno no grupo🙄'})
					break
				case 'dono':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/d1M6lOz.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* NINEM\n*YOUTUBE:* https://youtube.com/channel/UCm_QScsXSnxSRIu4ohz0xxQ\n*WPP:* wa.me/+5569993733829\n*INSTA:* kelvin_10156\n\n\n*Digite *.bot* para ver comandos basicos para criar um bot'})
					break
				case 'belle2':
					memein = await kagApi.memeindo()
				...
				case 'hentai':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Hentai é melhor que x.v 😎✋'})
					break
				case 'bomdia':
					memein = await kagApi.memeindo()
        ...
        case 'promote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Esse carinha aqui @${mentioned[0].split('@')[0]} agora é admin então respeitem ok? hihi! 😂`, mentioned, true)
				...
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Esse carinha aqui @${mentioned[0].split('@')[0]} Acabou de ser rebaixado para adm, pressionem F 😂!`, mentioned, true)
